...
_modify_docker_compose_prod_helper_scale() {
    ...
    for container_name in ${container_names_arr}; do
        ...
        echo "Service name that got is: ${service_name}"
        services+=("$service_name")
    done
    echo "Going to scale services: ${services[0]}, ${services[1]} to ${replicas_count} replicas each"
...
modify_docker_compose_prod_with_sleep() {
    ...
    container_names_list=$(ssh -o StrictHostKeyChecking=no "${machine}" "docker ps -f 'name=.*api|.*client'")
    container_names_list=$(echo "$container_names_list" | awk 'NR > 1 {print $NF}')
    echo "Container list is ${container_names_list}"
    container_one=$(echo "$container_names_list" | awk 'NR == 1 {print $0}')
    container_names_arr+=("$container_one")
    container_two=$(echo "$container_names_list" | awk 'NR == 2 {print $0}')
    container_names_arr+=("$container_two")
